...
> **Source:** <https://learn.microsoft.com/en-us/azure/confidential-computing/confidential-vm-faq>

*Answers to frequently asked questions (FAQs) about confidential virtual machines in Azure Confidential Computing.

---

This article provides answers to some of the most common questions about [confidential virtual machines (VMs)](confidential-vm-overview).
...
* Encryption for "data in useâ, including the processor state and the virtual machineâs memory. The keys are generated by the processor and never leave it.
...
Confidential VMs leverages AMD SEV-SNP with Azure technologies such as full-disk encryption and [Azure Key Vault Managed HSM](/en-us/azure/key-vault/managed-hsm/overview).
...
Confidential VMs combine Intel TDX with Azure technologies such as full-disk encryption and [Azure Key Vault Managed HSM](/en-us/azure/key-vault/managed-hsm/overview).
You can encrypt data in use, in transit, and at rest with keys that you control.
...
On confidential VMs with a temp disk, the pagefile.sys file can be moved to the encrypted OS [Tips for moving pagefile.sys to the c:\ drive](/en-us/azure/virtual-machines/windows/change-drive-letter).
...
* [Deploy from the Azure portal](quick-create-confidential-vm-portal)
* [Deploy from the Azure Command-Line Interface (Azure CLI)](quick-create-confidential-vm-arm)
* [Deploy using an ARM Template](quick-create-confidential-vm-arm)
...
Additionally, we support [IntelÂ® Trust Authority](https://www.intel.com/content/www/us/en/security/trust-authority.html) for enterprises seeking operator independent attestation.
Support for full in-guest [attestation](guest-attestation-confidential-vms), similar to AMD SEV-SNP is coming soon. This allows organizations to go deeper, and validate further aspects, even down to the guest application layer.
...
Yes. You can use [Azure Compute Gallery](/en-us/azure/virtual-machines/shared-image-galleries) to modify a confidential VM image, such as by installing applications.
...
However, you can also use other [disk encryption schemes](/en-us/azure/virtual-machines/disk-encryption-overview) along with or instead of full-disk encryption.
...
Azure free trial subscriptions don't have a large enough core quota for confidential VMs. [Create a support request to increase your quota](/en-us/azure/azure-portal/supportability/per-vm-quota-requests).
...
Lastly, for some high security and privacy settings, you might choose to create linked resources, such as a [Managed HSM Pool](/en-us/azure/key-vault/managed-hsm/overview).

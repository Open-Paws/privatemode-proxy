...
> **Source:** <https://learn.microsoft.com/en-us/azure/confidential-computing/concept-skr-attestation>

*Concept guide on what SKR is and its usage with Azure Confidential Computing Offerings*
...
* [Azure Key Vault Premium](../security/fundamentals/key-management)
* [Azure Key Vault Managed HSM](/en-us/azure/key-vault/managed-hsm/overview)
...
[Follow the details here for Az CLI based AKV creation](/en-us/azure/key-vault/general/quick-create-cli)

Make sure to set the value of [--sku] to "premium".

### Step 2: Create a Secure Key Release Policy

A Secure Key Release Policy is a json format release policy as defined [here](/en-us/rest/api/keyvault/keys/create-key/create-key?tabs=HTTP#keyreleasepolicy) that specifies a set of claims required in addition to authorization to release the key. The claims here are MAA based claims as referenced [here for SGX](/en-us/azure/attestation/attestation-token-examples#sample-jwt-generated-for-sgx-attestation) and here for [AMD SEV-SNP CVM](/en-us/azure/attestation/attestation-token-examples#sample-jwt-generated-for-sev-snp-attestation).

Visit the TEE specific [examples page for more details](skr-policy-examples). For more information on the SKR policy grammar, see [Azure Key Vault secure key release policy grammar](/en-us/azure/key-vault/keys/policy-grammar).
...
Exact details of the type of key and other attributes associated can be found [here](/en-us/azure/key-vault/general/quick-create-cli).
...
* [Application within AMD EV-SNP based CVM's performing Secure Key Release](skr-flow-confidential-vm-sev-snp)
* [Confidential containers with Azure Container Instances (ACI) with SKR side-car containers](skr-flow-confidential-containers-azure-container-instance)
...
Yes, detailed examples by TEE type are listed [here.](skr-policy-examples)
...
[Azure Container Instance with confidential containers Secure Key Release with container side-cars](skr-flow-confidential-containers-azure-container-instance)

[CVM on AMD SEV-SNP Applications with Secure Key Release Example](skr-flow-confidential-vm-sev-snp)

[AKV REST API With SKR Details](/en-us/rest/api/keyvault/keys/create-key/create-key?tabs=HTTP)

[Azure Key Vault secure key release policy grammar](/en-us/azure/key-vault/keys/policy-grammar)

[AKV SDKs](/en-us/azure/key-vault/general/client-libraries)
